...
internal abstract class Channel: AbstractChannel
{
	...
	public static async Task<ChannelDto> MakeDto(Channel channel, UserChannelConfiguration? askingProfileConfig, GetEnsDomain? getEnsDomain)
	{
		...
		return new ChannelDto(channel.Id,
							  channelMembers,
							  type,
							  askingProfileConfig?.NumberUnreadMessages ?? 0,
							  channel.ApplicationId,
							  channel.IsPublic,
							  channel.Name,
							  channel.ImageUri != null? channel?.ImageUri.ToString() : null);
	}
...
internal sealed record ChannelMember(ProfileDto Profile, Permission Permission);